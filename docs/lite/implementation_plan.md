...
- `local_codex.py`: thin adapter around `codex exec`.
- `schemas.py`: JSON schemas for local Codex structured outputs.
...
The first model-backed component replaces `FullPageFallbackInventory` with a
local Codex provider that:

1. Sends rendered page images to `codex exec --image`.
...
Codex-backed visual inventory:

```bash
python -m src.lite.cli papers/ChangeGuard.pdf --output-dir outputs/lite --provider codex
```
